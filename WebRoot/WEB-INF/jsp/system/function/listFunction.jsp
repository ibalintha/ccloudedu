				...
				<tr>
					<td>
						 <div align="center" id="uldiv">
							...
							<ul >
								<li><a href="${ctx }/system/func_update.do?id=${id }"><span><my:i18n zhText="菜单信息" enText="Menu Info"/></span></a></li>
								<li id="current"><a href="${ctx }/system/func_list.do?id=${id }"><span><my:i18n zhText="下级菜单列表" enText="Sub Menu List"/></span></a></li>
			  ...
			  <table class="ltable" width="100%">
			        <thead class="ltablehead">
			           ...
			           <c:forEach items="${page.list}" var="func" varStatus="status">
			               <tr id="${func.id }">
								<td align="center"><input type="checkbox" name="ids"  value="${func.id}"/></td>
								<td align="center"><my:rowNum page="${page}" rowIndex="${status.index}"/></td>
								<td>
								   <a href="${ctx }/system/func_update.do?id=${func.id}" name="menuDetail"><my:i18n zhText="${func.chFuncName}" enText="${func.chFuncName}"/></a>
								</td>
								<td>${func.chFuncPath}</td>
								<%--<td align="center">${menu.remark}</td> --%>
								<td align="center">
									<my:auth fixedValue="W" value="${auth}"><a href="javascript:" class="deleteOne" param="ids=${func.id}"><my:i18n zhText="删除" enText="Delete"/></a></my:auth>
								</td>
							</tr>
			           </c:forEach>
		             </tbody>
