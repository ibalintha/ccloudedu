...
<html>
	...
	var valueclass = obj.options[obj.options.selectedIndex].text;
		...
		$.ajax({
						url: "student_classReport.do",
						type: 'post',
						data: {"valueclass" : valueclass},				
						success: function(data) {
						var obj = $.parseJSON(data);	
						//alert(obj);		
						$('#myframe').attr("src","student_exportByclass.do");		
						},
				});
}
	function gradegetSelectedOption(){
		...
		function exportPreview(n){
				...
				window.location.href="student_exportPdf.do?exportId="+n;
				method = "post";  		
		...
		function exportexcel(n){
			...
			{
				//alert("相等1");
				window.location.href="student_exportExcelStuAll.do";
			...
			{
				window.location.href="student_exportExcel.do";
				//$('#myframe').attr("src","student_exportExcel.do");
			}
			else if('3' == n)
			{
				window.location.href="student_exportExcelCount.do";
			}
		}
					...
					<td width="20%" style="border: 2px;">
						<table bgcolor="#EFEFEF" width="100%" height="100%">
							...
							<tr height="30px">
								...
								<td width="60%">
									<select name="studentList" id="reportList_id"
										style="width: 100px;">
										<option value="">
											--请选择--
										</option>
